...
const checkValues = (obj, value) => {
  ...
  arr.forEach(objValue => {(objValue === value)? boolValue=true: boolValue=false} );
  return boolValue ;
};
  /* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const updateNumbers = (obj) => {
  // Solution code here...
  const newArray = Object.keys(obj).map(key => `${String(key)}: ${String(obj[key])}`);
  return newArray ;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  let tORf = false ;
  arr.forEach(objetc => {
    if(Object.values(objetc).indexOf(character) !== -1) 
      if(Object.keys(objetc).indexOf('children') !== -1) tORf=true;
  } );
  return tORf ;
};
