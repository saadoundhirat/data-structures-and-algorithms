...
const count = (target, input) => {
  ...
  return input.reduce((acc, item) =>{
    return (
      acc + item.reduce((acc, item) => {return (item === target ? acc+1 : acc);}, 0)
      );}, 0);
};
...
const totalSum = (input) => {
  ...
  let output= input.reduce((acc, element)=>{
    let out = element.reduce((accumulator, item)=>{
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((result) => result.gender === 'male' || result.gender === 'female').map((character) => character.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let value = data.filter( char => char.height).reduce((a, b) => (b.height > a.height ? b : a));
  return value.name;
};
